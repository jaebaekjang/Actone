...
## 0-1. 디자인 시스템 적용 완료 (2026-07-08)

V3 SPOTLIGHT 맨데이트 구현됨 — 스포트라이트 히어로, V3 팔레트, 행 기반 게시판,
라이트 관리자 UI, 디자인 문서 3종, 스크린샷 QA(390/768/1440). 자세한 상태와 남은
디자인 이슈는 `ACTONE_DESIGN_QA.md`의 "남은 문제" 참고. **디자인 언어 변경 금지.**
비주얼 재검수는 `e2e/design-shots.mjs`로 재캡처 가능.
...
- (완료됨 2026-07-09) ~~글 수정 시 제거 이미지 Storage 정리~~ — `updatePost`가 제거된
  URL의 스토리지 파일을 `post-images` 버킷에서 삭제 (`apps/web/lib/actions/posts.ts`).
  실제 Supabase Storage에서의 삭제 동작은 실인스턴스 검증 시 확인 필요(로컬 스택은 storage 스텁).
